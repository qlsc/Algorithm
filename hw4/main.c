...
static void getargs(char cmd[], int *argcp, char *argv[])
{
    ...
    while ( (cmdp = getword(cmdp, &end)) != NULL && *cmdp != '#') { /* end is output param */  //fix bug 1
...
{
    signal(SIGINT,interrupt_handler); //fix bug 3
    
    if (argc > 1) freopen(argv[1], "r", stdin); //fix bug 2
